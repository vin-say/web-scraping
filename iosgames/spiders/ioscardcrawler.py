...
class Ioscardcrawler(CrawlSpider):
    
    name = 'ioscardcrawler'
    allowed_domains = ['apps.apple.com']
    start_urls = ['https://apps.apple.com/us/genre/ios-games-card/id7005?letter=A']

    rules = (
        # paginate by letter
        Rule(LinkExtractor(allow = 'genre/ios-games-card/id7005\?letter=(\D)')),
        # paginate to next page
        Rule(LinkExtractor(allow = 'genre/ios-games-card/id7005\?letter=(\D)&page=(\d+)#page', restrict_xpaths = '//a[@class="paginate-more"]')),
