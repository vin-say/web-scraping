...
class Iosboardcrawler(CrawlSpider):
    
    name = 'iosboardcrawler'
    allowed_domains = ['apps.apple.com']
    start_urls = ['https://apps.apple.com/us/genre/ios-games-board/id7004?letter=A']

    rules = (
        # paginate by letter
        Rule(LinkExtractor(allow = 'genre/ios-games-board/id7004\?letter=(\D)')),
        # paginate to next page
        Rule(LinkExtractor(allow = 'genre/ios-games-board/id7004\?letter=(\D)&page=(\d+)#page', restrict_xpaths = '//a[@class="paginate-more"]')),
