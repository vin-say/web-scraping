...
class Iosfamilycrawler(CrawlSpider):
    
    name = 'iosfamilycrawler'
    allowed_domains = ['apps.apple.com']
    start_urls = ['https://apps.apple.com/us/genre/ios-games-family/id7009?letter=A']

    rules = (
        # paginate by letter
        Rule(LinkExtractor(allow = 'genre/ios-games-family/id7009\?letter=(\D)')),
        # paginate to next page
        Rule(LinkExtractor(allow = 'genre/ios-games-family/id7009\?letter=(\D)&page=(\d+)#page', restrict_xpaths = '//a[@class="paginate-more"]')),
